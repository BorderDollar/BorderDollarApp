...
fn create_crowdfund_contract(
    ...
    crowdfund
        .client()
        .init(recipient, &deadline, target_amount, token);
    (id, crowdfund)
}
...
#[test]
fn test_deposit_and_contributors() {
    let setup = Setup::new();
    ...
    let contributors: Vec<Address> = setup.crowdfund.client().contributors();

    // Verify the contributors list
    assert_eq!(contributors.len(), 2);

    assert_eq!(setup.crowdfund.client().balance(&setup.user2), 3);
